/**
 * CreditAddedSheet — paper-light celebration for a successful extra-
 * credit purchase. Same choreography family as UpgradedSheet: staggered
 * reveal, accent-coloured editorial title, hairline bloom, ink-stamp dot.
 * Same total duration (~1.4s) so both happy paths feel like siblings.
 *
 * The credit moment is smaller than a tier upgrade, but the brand
 * vocabulary (paper, accent display, signature ink stamp) stays the same
 * so the user reads "good thing happened" the same way both times.
 *
 * Dismissal navigates to Generate, since the obvious next step after
 * buying a credit is to spend it.
 */
...
interface Props {
  visible: boolean;
  /** Total credit count after the purchase. */
  creditsAfter: number;
  onDismiss: () => void;
}
...
export function CreditAddedSheet({
  visible,
  creditsAfter,
  onDismiss,
}: Props) {
  ...
  const body =
    creditsAfter === 1
      ? "You have 1 credit. Tap a topic and we'll get researching."
      : `You have ${creditsAfter} credits. Tap a topic and we'll get researching.`;
        ...
        <SafeAreaView style={styles.sheet} edges={["left", "right", "bottom"]}>
          ...
          <Animated.View style={[styles.footer, fadeUpStyle(ctaAnim)]}>
            <Pressable
              ...
              accessibilityLabel="Start a podcast"
              style={({ pressed }) => [
                styles.cta,
                pressed && styles.ctaPressed,
              ]}
            >
              <Text style={styles.ctaLabel}>Start a podcast</Text>
            </Pressable>
          </Animated.View>
        </SafeAreaView>
      </View>
